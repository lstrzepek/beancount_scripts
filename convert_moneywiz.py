...
def build_records(mapping, record):
    def description_and_tags(desc, tags):
        if tags:
            ...
            return '"%s"' % desc
    name, _, account, transfers_to, description, payee, category, date, amount, currency, _, tags = record
...
def print_mappings(records):
    account_names = set()
    categories = set()
    tag_names = set()
    for record in records:
        name, _, _, _, _, _, category, _, _, _, _, tags = record
        if tags:
            tags = tags.split(";")
            for tag in tags:
                if tag.strip():
...
if __name__ == '__main__':
    ...
    load_dotenv() #reads .env file in current dir
